...
\end{equation}
using the divergence theorem to write the first term as a surface integral. If instead of treating $\bm{f}$ as a quantity immediately determined by $\bm{u}$ and $\gamma$ we treat it as simply the unknown force exerted on the membrane by the bulk, we can write down the Evolving-Stokes Rayleighian augmented by additional terms corresponding to the bulk dissipation and permeative dissipation,
\begin{multline}
    \mathcal{R}[\bm{u},\bm{f},\gamma] = \\\int\braket{-\bm{L}\bm{u}|\bm{u}}dS+\int\braket{\delta_\varphi V|\bm{u}}dS+\gamma\dot{A}-\int\braket{\bm{f}|\bm{u}}dS+\frac{1}{2}\int\braket{\bm{f}|(\bm{u}^\infty+S[\bm{f}])}dS+\frac{\beta}{2}\int(\Gamma-\bm{f}\cdot\bm{n})^2dS,
\end{multline}
where $\bm{L}$ is the symmetric, negative-definite viscosity laplacian operator. Taking variations with respect to our three unknowns, we get the stationary conditions
\begin{align}
...
\end{align}
Respectively, these equations are our boundary integral equation (19), the Evolving-Stokes equations, and the (global) incompressiblity constraint.

\subsection*{Optimization scheme and feasibility}
In practice, our scheme achieves stability by solving for the prospective future shape $\varphi^{(n+1)}$ which minimizes the Rayleighian on the original geometry $\varphi^{(n)}$. Thus we can hold fixed the operators on $\varphi_0$ and solve the optimization problem
\begin{multline}
    \min_{\varphi^{(n+1)},f^{(n)}}\bigg[\braket{\braket{-\bm{L}^{(n)}(\varphi^{(n+1)}-\varphi^{(n)})|(\varphi^{(n+1)}-\varphi^{(n)})}}+ dt\int (H^{(n+1)})^2-(H^{(n)})^2dS \\ -dt\braket{\braket{f^{(n)}|(\varphi^{(n+1)}-\varphi^{(n)})}}+\frac{dt^2}{2}\braket{\braket{f^{(n)}|\big(u^{\infty}+\sd{}\bm{S}^{(n)}f^{(n)}\big)}}+\frac{dt^2}{2}\sd{}\da{}\int_S\big(\Gamma-f^{(n)}\cdot n^{(n)}\big)^2\bigg]
\end{multline}
...
\begin{gather}
\bm{H}=
 \begin{bmatrix} -\bm{L}^{(n)}+dt\bm{\Delta}^{(n)}\bm{\Delta}^{(n)}& -\bm{I} \\ -\bm{I} & dt^2\sd{}(\bm{S}^{(n)}+\da{}\bm{n}^{(n)}\bm{n}^{(n)}) \end{bmatrix}
 \end{gather}
 and is entirely determined by the geometry. Is this matrix positive definite, and thus is our problem convex? To be convex,
\begin{equation}
    z^THz > 0\text{ for all } \bm{z}.
\end{equation}
Picking our geometry (and thus $H$) and $z$ wisely, we can prove there are situations for which this condition is not satisfied. Consider a vector field on the unit sphere composed of magnetic/toroidal vector spherical harmonic modes:
\begin{equation}
    z(\phi,\theta) = \sum_{\ell=0}^\infty\sum_{m=-\ell}^\ell z_{\ell m}\bm{T}_{\ell m},\quad \bm{T}_{\ell m}=\bm{n}\times\nabla_s Y_{\ell m}
\end{equation}
The Bochner laplacian $\bm{\Delta}$ is a negative-definite operator defined as 
\begin{equation}
    \bm{\Delta} = \nabla^\alpha\nabla_\alpha.
\end{equation}
We note that, since $\bm{T}_{\ell m}$ are tangent vector fields,
\begin{equation}
    \bm{T}_{\ell m}=\bm{n}\times\nabla_s Y_{\ell m}=J\nabla_s Y_{\ell m}
\end{equation}
where $J$ is a 90-degree rotation in the tangent plane. Now we can use the commutation formula for the Laplacian and gradient, 
\begin{equation}
    J\nabla^\alpha\nabla_\alpha\nabla^\beta Y_{\ell m} = J(\nabla^\beta\Delta_sY_{\ell m} + g^{\beta\mu}Ric_{\mu\alpha}\nabla^\alpha Y_{\ell m}).
\end{equation}
On the unit sphere, the Ricci tensor is just the metric tensor, and by the definition of scalar spherical harmonics $\Delta_sY_{\ell m}=-\ell(\ell+1)Y_{\ell m}$, which leaves us with 
\begin{align}
    \nabla^\alpha\nabla_\alpha\nabla^\beta JY_{\ell m} &= (1-\ell(\ell+1))JY_{\ell m} \\
    \bm{\Delta}\bm{T}_{\ell m}&=(1-\ell(\ell+1))\bm{T}_{\ell m}.
\end{align}
Now for the full viscosity Laplacian, 
\begin{equation}
    -\bm{L}\bm{T}_{\ell m} = (\bm{\Delta}+K+grad\circ div)\bm{T}_{\ell m} + 2\langle \nabla \bm T_{\ell m},II\rangle\, \bm{n}.
\end{equation}
Since $\nabla\cdot\bm{T}=0$ the last two terms go away and using the uniform $K=1$ on the unit sphere, 
\begin{equation}
    \bm{L}\bm{T}_{\ell m}=(2-\ell(\ell+1))\bm{T}_{\ell m}.
\end{equation}
The eigenvalues of the Stokes single-layer operator for vector spherical harmonics have been worked out by Veerapaneni et al. (2017) and the result for the magnetic mode is that 
\begin{equation}
    \bm{S}\bm{T}_{\ell m}=\frac{1}{2\ell+1}\bm{T}_{\ell m}.
\end{equation}
And clearly, as the toroidal modes are tangential, the normal projection operator $\bm{n}\bm{n}^T$ has eigenvalue $0$. Thus in the subspace of toroidal VSH modes, our Hessian can be expressed simply as 
\begin{gather}
\bm{H}=
 \begin{bmatrix} \ell(\ell+1)-2+dt(1-\ell(\ell+1))^2& -1 \\ -1 & \frac{dt^2}{2\ell+1} \end{bmatrix}.
 \end{gather}
Thus, the Hessian has negative eigenvalues when
$$
\frac{dt^2(\ell(\ell+1)-2)+dt^3(1-\ell(\ell+1))^2}{2\ell+1}-1<0
$$
which means the smallest eigenvalue will be negative when the timestep $dt<1.44$. This proves that the Hessian (35) on a fairly common geometry, the unit sphere, is not positive definite and thus our proposed optimization problem is non-convex.
