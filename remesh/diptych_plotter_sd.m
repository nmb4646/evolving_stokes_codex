...
% Diptych movie for two Sd values with fixed gamy and Da.
...
run_tags = strings(size(Sd_values));
for k = 1:numel(Sd_values)
    run_tags(k) = make_run_tag(gamy, Da, Sd_values(k));
end

name = "./data/diptych_Sd";
delete(name)
v = VideoWriter(name);
v.FrameRate = 14;
open(v)
...
last_frames = zeros(size(Sd_values));
for k = 1:numel(Sd_values)
...
curvature_min = inf(1, numel(Sd_values));
curvature_max = -inf(1, numel(Sd_values));
flux_abs_max = ones(1, numel(Sd_values));
...
for n = ti:ts:tf
    ...
    for k = 1:numel(Sd_values)
        ...
        geoj_Geom = Geometry(geoj.M, geoj.P);
        disp(sprintf("Sd = %.2f, volume = ",Sd_values(k)) + geoj_Geom.volume)
        ...
        axis off;
        grid off;
        title(sprintf('Sd = %.2f', Sd_values(k)));
    end
